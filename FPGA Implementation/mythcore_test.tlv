...
\SV
   ...
   //   library file (or check with Redwood EDA regarding recent developments).
   //
   // This file contains:
   ...
   m4_makerchip_module   // Compile within Makerchip to see expanded module definition.
      reg [7:0] out; // To check output on LEDs of Zedboard.
      
      core uut (clk, reset, out);
      
      assign passed = out == (1+2+3+4+5+6+7+8+9);
      assign failed = 1'b0;
...
// /====================\
   ...
   \SV_plus
      always @ (posedge clk) begin
         if (|cpu/xreg[17]>>5$value ==? (1+2+3+4+5+6+7+8+9))
